...
const Cadastro = () => {
    ...
    const formSchema = useMemo(() => z.object({
        ...
        phone: z.string().min(5, "Telefone muito curto"),
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            // Combine countryCode and phone for the payload if needed, or send as separate fields.
            // Sending as a combined "phone" field to match expected webhook format usually.
            const fullPhone = `${values.countryCode} ${values.phone}`;
            const payload = {
                ...values,
                phone: fullPhone, // Override phone with full international format
                variant
            };
            ...
            if (response.ok) {
                ...
                form.reset();
                window.location.href = "https://chat.whatsapp.com/FSTWzjvLzT1Axi4NZenI60?mode=gi_c";
            } else {
                        ...
                        <div className="order-2 lg:order-1">
                            ...
                            <div className="space-y-8 animate-stagger">
                                ...
                                <div className="flex gap-4 stagger-item opacity-0">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-semibold mb-2">6 Estratégias Automatizadas</h3>
                                        <p className={`leading-relaxed ${isDark ? "text-muted-foreground" : "text-slate-600 font-medium opacity-80"}`}>
                                            Portfólio estruturado com lógica complementar entre os robôs, operando sem fator emocional.
                                        </p>
                                    </div>
                                </div>
                                <div className="flex gap-4 stagger-item opacity-0">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-semibold mb-2">Teste Grátis</h3>
                                        <p className={`leading-relaxed ${isDark ? "text-muted-foreground" : "text-slate-600 font-medium opacity-80"}`}>
                                            ...
                                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                                                            ...
                                                            <FormControl>
                                                                <div className="flex gap-2">
                                                                    <CountrySelector
                                                                        ...
                                                                        className={!isDark ? "bg-white border-slate-300 text-slate-900" : ""}
                                                                    />
                                                                    <Input
                                                                        placeholder={form.watch("countryCode") === "+55" ? "(11) 99999-9999" : "99999-9999"}
                                                                        {...field}
                                                                        className={!isDark ? "bg-white border-slate-300 text-slate-900 focus:ring-primary focus:border-primary shadow-sm" : ""}
                                                                        onChange={(e) => {
                                                                            let v = e.target.value.replace(/\D/g, "");
                                                                            const currentCountry = form.watch("countryCode");

                                                                            if (currentCountry === "+55") {
                                                                                // Brazilian Mask
                                                                                v = v.replace(/^(\d{2})(\d)/g, "($1) $2");
                                                                                v = v.replace(/(\d)(\d{4})$/, "$1-$2");
                                                                                if (v.length > 15) v = v.substring(0, 15);
                                                                            } else {
                                                                                // International: Just limit length reasonably
                                                ...
                                                <Button type="submit" className="w-full mt-2" disabled={isLoading} size="lg">
                                                    {isLoading ? (
                                                        ...
                                                        "Adquirir os Robôs Gratuitamente"
                                                    )}
                                                </Button>
                                            </form>
                                        </Form>
